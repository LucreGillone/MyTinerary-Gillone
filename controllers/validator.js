...
const validator = (req, res, next) => {
    const schema = joi.object({
        ...
        firstName: joi.string().trim().min(2).required().messages({
            ...
            "string.min": "The field 'First Name' must have at least 2 characters",
        }),
        lastName: joi.string().min(2).required().messages({
            "string.empty": "The field 'Last Name' is required",
            "string.min": "The field 'Last Name' must have at least 2 characters"
        }),
        src: joi.string(),
        country: joi.string(),
        email: joi.string().trim().email().min(3).messages({
            "string.min": "Your email must have at least 3 characters",
            "string.pattern.base": "You must write a valid email adress",
        }),
